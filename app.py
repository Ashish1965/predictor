...
def index():
    ...
    if request.method == 'POST':
        ...
        current_score_str = request.form.get('current_score', '').strip()

        if current_score_str:
            current_score = int(current_score_str)
        else:
            # Handle the case where current_score is empty or missing
            current_score = 0  # or any default value or error handling
